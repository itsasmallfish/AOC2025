const fs = require('fs');

const Method = {
    POINT_COUNTER: 'POINT_COUNTER',
    PASSING_COUNTER: 'PASSING_COUNTER'
}

const DIAL_SIZE = 100
const PASSWORD_METHOD = Method.PASSING_COUNTER

let pointer = 50
let password_counter = 0

function shift(instruction) {
    const direction = instruction[0]
    const amount = parseInt(instruction.slice(1), 10)
    for (let i = 0; i < amount; i++) {
        move(direction)
    }
    if (PASSWORD_METHOD == Method.POINT_COUNTER && pointer == 0) {
        password_counter++
    }
}

function move(direction) {

    switch (direction) {
        case "R":
            pointer += 1
            break;
        case "L":
            pointer -= 1
            break;
    }
    pointer = pointer % DIAL_SIZE
    if (pointer < 0) {
        pointer += DIAL_SIZE
    }
    if (PASSWORD_METHOD == Method.PASSING_COUNTER && pointer == 0) {
        password_counter++
    } 
}

try {
    const data = fs.readFileSync("day01_input01.txt", "utf8")
    const instructions = data.split("\n")
    for (const instruction of instructions) {
        if (instruction) {
            shift(instruction)
        }
    }
} catch (error) {
    console.error("Error reading the file:", error)
}

console.log(password_counter)
